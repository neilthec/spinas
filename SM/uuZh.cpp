  ...
  cdouble uuZh::amp(const int& ds1, const int& ds2, const int& ds3){//Double Spin
    ...
    for(int i=0;i<nCombs;i++){
      ...
      amplitude += normFactor*preTU*(
				     gR*s13s.v(ds1,ds3a)*(-2.0*mu*a23a.v(ds2,ds3b)+s243a.v(ds2,ds3b))
				     +gL*a13a.v(ds1,ds3a)*(-2.0*mu*s23s.v(ds2,ds3b)+s342a.v(ds3b,ds2))
				     )/pDenT;
      ...
      amplitude += normFactor*preTU*(
				     gL*s23s.v(ds2,ds3a)*(s143a.v(ds1,ds3b)-2.0*mu*a13a.v(ds1,ds3b))
				     + gR*a23a.v(ds2,ds3a)*(s341a.v(ds3b,ds1)-2.0*mu*s13s.v(ds1,ds3b))
				     )/pDenU;
    }
