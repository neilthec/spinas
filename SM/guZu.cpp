...
namespace spinas {
  ...
  cdouble guZu::amp(const int& ds1, const int& ds2, const int& ds3, const int& ds4){//Double Spin
	...
	//guZU::
	//+ preTU (gLu<23>(-1MZ[14][123>+<>Mu<>[13][134>)+gRu<34>(<>Mu<>[13][132>+-1MZ[12][143>))
	...
	amplitude += normFactor*preTU*(
				       gL*a23a.v(ds2,ds3a)*(-MZ*s14s.v(ds4)*s123a.v(ds3b)-mu*s13s.v(ds3b)*s134a.v(ds4))
				       +gR*a34a.v(ds3a,ds4)*(-mu*s13s.v(ds3b)*s132a.v(ds2)-MZ*s12s.v(ds2)*s143a.v(ds3b))
				       )/pDenU/pDenS;
	...
	amplitude += normFactor*preTU*(
				       gR*s23s.v(ds2,ds3a)*(-MZ*a14a.v(ds4)*a123s.v(ds3b)-mu*a13a.v(ds3b)*a134s.v(ds4))
				       +gL*s34s.v(ds3a,ds4)*(-mu*a13a.v(ds3b)*a132s.v(ds2)-MZ*a12a.v(ds2)*a143s.v(ds3b))
				       )/pDenU/pDenS;
