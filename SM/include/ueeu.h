...
//File:  SPINAS/SM/ueeu.h

namespace spinas {

  class ueeu : public process {
  private:
    ldouble e;//Electric Charge
    ldouble mu, me, mh, wh, MZ, WZ, MW, SW, CW;//Mass of u, s, Higgs, width of Higgs, Z mass, Z width, W-boson mass and sin and cos of the Weinberg angle.
    particle p1,p2,p3,p4;
    propagator propA, proph, propZ;
    cdouble pDenUA, pDenUh, pDenUZ;
    sproduct a13a, s13s, a14a, s14s, a23a, s23s, a24a, s24s;
    sproduct s12s, a12a, s34s, a34a;
    ldouble preh, gLu, gRu, gLe, gRe, preZ, preZ0;

    
  public:
    ueeu(const ldouble& echarge,
	 const ldouble& massu, const ldouble& masse,
	 const ldouble& massh, const ldouble& widthh,
	 const ldouble& massW, const ldouble& sinW, const ldouble& widthZ);

    //Set Masses
    void set_masses(const ldouble& massu, const ldouble& masse, const ldouble& massh, const ldouble& massW);
    //Set Momenta
