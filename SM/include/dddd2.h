...
//File:  SPINAS/SM/dddd2.h

namespace spinas {

  class dddd2 : public process {
  private:
    ldouble e, gs;//Electric Charge
    ldouble md, mh, wh, MZ, WZ, MW, SW, CW;//Mass of d, Higgs, width of Higgs, Z mass, Z width, W-boson mass and sin and cos of the Weinberg angle.
    particle p1,p2,p3,p4;
    propagator propAG, proph, propZ;
    cdouble pDenTAG, pDenTh, pDenTZ;
    cdouble pDenUAG, pDenUh, pDenUZ;
  ...
  public:
    dddd2(const ldouble& echarge, const ldouble& gscharge,
	 const ldouble& massd,
    ...
    cdouble amp_gluon_U(const int& ds1, const int& ds2, const int& ds3, const int& ds4);//Double the spins
    cdouble amp_rest_U(const int& ds1, const int& ds2, const int& ds3, const int& ds4);//Double the spins
    ldouble amp2();
