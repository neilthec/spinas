...
//File:  SPINAS/SM/ddss.h

namespace spinas {

  class ddss : public process {
  private:
    ldouble e, gs;//Electric Charge
    ldouble md, ms, mh, wh, MZ, WZ, MW, SW, CW;//Mass of u, c, Higgs, width of Higgs, Z mass, Z width, W-boson mass and sin and cos of the Weinberg angle.
  ...
  public:
    ddss(const ldouble& echarge, const ldouble& gscharge,
	 const ldouble& massd, const ldouble& masss,
	 const ldouble& massh, const ldouble& widthh,
	 const ldouble& massW, const ldouble& sinW, const ldouble& widthZ);

    //Set Masses
    void set_masses(const ldouble& massd, const ldouble& masss, const ldouble& massh, const ldouble& massW);
    //Set Momenta
