...
//File:  SPINAS/SM/uucc.h

namespace spinas {

  class uucc : public process {
  private:
    ldouble e, gs;//Electric Charge
    ldouble mu, mc, mh, wh, MZ, WZ, MW, SW, CW;//Mass of u, c, Higgs, width of Higgs, Z mass, Z width, W-boson mass and sin and cos of the Weinberg angle.
  ...
  public:
    uucc(const ldouble& echarge, const ldouble& gscharge,
	 const ldouble& massu, const ldouble& massc,
	 const ldouble& massh, const ldouble& widthh,
	 const ldouble& massW, const ldouble& sinW, const ldouble& widthZ);

    //Set Masses
    void set_masses(const ldouble& massu, const ldouble& massc, const ldouble& massh, const ldouble& massW);
    //Set Momenta
